...
const _require5 = require(`../../utils/js-chunk-names`),
      generateComponentChunkName = _require5.generateComponentChunkName;
...
actions.createPage = (page, plugin, actionOptions) => {
  ...
  if (_.isObject(page.context)) {
    ...
    if (invalidFields.length > 0) {
      const error = `${invalidFields.length === 1 ? singularMessage : pluralMessage}
...
${reservedFields.map(f => `  * "${f}"`).join(`\n`)}
  ...
  if (process.env.NODE_ENV !== `test`) {
    if (!fileExistsSync(page.component)) {
      const message = `${name} created a page with a component that doesn't exist. Missing component is ${page.component}`;
      console.log(``);
      console.log(chalk.bold.red(message));
      console.log(``);
      console.log(page);
  ...
  if (!page.component || !path.isAbsolute(page.component)) {
    const message = `${name} must set the absolute path to the page component when create creating a page`; // Don't log out when testing
  ...
  let internalPage = {
    ...
    isCreatedByStatefulCreatePages: actionOptions && actionOptions.traceId === `initial-createPagesStatefully`,
    // Ensure the page has a context object
    context: page.context || {},
    updatedAt: Date.now() // If the path doesn't have an initial forward slash, add it.
  ...
  if (!internalPage.component.includes(`/.cache/`) && process.env.NODE_ENV === `production` && !fileOkCache[internalPage.component]) {
    ...
    if (!fileContent.includes(`export default`) && !fileContent.includes(`module.exports`) && !fileContent.includes(`exports.default`) && // this check only applies to js and ts, not mdx
    /\.(jsx?|tsx?)/.test(path.extname(fileName))) {
      includesDefaultExport = false;
    }

    if (!notEmpty || !includesDefaultExport) {
      const relativePath = path.relative(store.getState().program.directory, fileName);

      if (!notEmpty) {
        report.panicOnBuild(`You have an empty file in the "src/pages" directory at "${relativePath}". Please remove it or make it a valid component`);
      }

      if (!includesDefaultExport) {
        report.panicOnBuild(`[${fileName}] The page component must export a React component for it to be valid`);
      }
    }
...
actions.deleteNode = (options, plugin, args) => {
  ...
  const deleteDescendantsActions = node && findChildrenRecursively(node.children).map(getNode).map(createDeleteAction);
...
actions.deleteNodes = (nodes, plugin) => {
  ...
  const descendantNodes = _.flatten(nodes.map(n => findChildrenRecursively(getNode(n).children)));
...
actions.createNode = (...args) => dispatch => {
  ...
  const node = createNodeAction.payload,
        traceId = createNodeAction.traceId,
        parentSpan = createNodeAction.parentSpan;
